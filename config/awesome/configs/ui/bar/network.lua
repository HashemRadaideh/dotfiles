...
local function get_interface_names()
  local interface_names = {}

  local lan_interfaces = execute_command("ip -o link show | awk -F': ' '{if ($2 ~ /^e|^en/) print $2}'")
  for lan_interface in lan_interfaces:gmatch("[^\r\n]+") do
    interface_names["lan"] = lan_interface
  end

  local wlan_interfaces = execute_command("iw dev | awk '$1==\"Interface\"{print $2}'")
  for wlan_interface in wlan_interfaces:gmatch("[^\r\n]+") do
    interface_names["wlan"] = wlan_interface
  end
...
local return_button = function()
  ...
  local update_wireless = function()
    ...
    local update_wireless_data = function(strength, healthy)
      awful.spawn.easy_async_with_shell([[ iw dev ]] .. network_interfaces.wlan .. [[ link ]], function(stdout)
  ...
  function check_network_directory() {
    if [[ -n "${wireless}" && -d "${net}${wireless}" ]]; then
